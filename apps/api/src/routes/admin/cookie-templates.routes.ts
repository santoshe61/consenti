  ...
  return {
    ...
    'PUT /cookie-templates/:id': async (req: Request, params: Record<string, string>): Promise<Response> =>
      withErrorHandler(async () => {
        ...
        // Cookie removal validation: if cookies were removed, check against profiles' compliance rules
        if (Array.isArray(body?.cookies)) {
          const oldTemplate = await storage.getCookieTemplate(id)
          if (oldTemplate) {
            const newCookieIds = new Set((body.cookies as Cookie[]).map(c => c.id))
            const removedIds = oldTemplate.cookies.map(c => c.id).filter(cid => !newCookieIds.has(cid))
            if (removedIds.length > 0) {
              const profiles = await storage.findProfilesUsingCookieTemplate(id)
              const blockingProfiles: { id: string; name: string; complianceGroup: string }[] = []
              for (const p of profiles) {
                if (!p.complianceGroup) continue
                const result = validateProfileCompliance(body.cookies as Cookie[], p.complianceGroup)
                if (!result.valid) {
                  blockingProfiles.push({ id: p.id, name: p.name, complianceGroup: p.complianceGroup })
                }
              }
              if (blockingProfiles.length > 0) {
                return errorResponse(422, 'Removing these cookies would break compliance for affected profiles', {
                  blockingProfiles,
                  removedCookieIds: removedIds,
                })
              }
            }
          }
        ...
        const t = await storage.updateCookieTemplate(id, update)
        return json(200, t)
      }),
    ...
    'DELETE /cookie-templates/:id': async (req: Request, params: Record<string, string>): Promise<Response> =>
      withErrorHandler(async () => {
        ...
        await storage.deleteCookieTemplate(id)
    ...
    'GET /cookie-templates/:id/profile-usage': async (req: Request, params: Record<string, string>): Promise<Response> =>
      withErrorHandler(async () => {
        const { denied } = await auth(req, 'profile:view')
        if (denied) return denied
        const usage = await storage.findProfilesUsingCookieTemplate(params['id']!)
