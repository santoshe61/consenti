...
import { Tag, Scale, Lock, Globe, Calendar, Building2, ShieldCheck } from 'lucide-react'
...
function emptyRow(): TemplateCookie {
  return { id: '', type: 'consent', mandatory: false, listenGpc: true, expiry: 365 }
}
...
const FIELD_INFO: Array<{
  ...
  usage: string; laws: Array<{ name: string; text: string }>; examples: string
}> = [
    {
      field: 'ID',
      label: 'Cookie ID',
    ...
    },
    {
      ...
      examples: 'Use "Consent" for analytics, advertising, A/B testing. Reserve "Legitimate Interest" only for strictly-server-side processing with legal review.',
    },
    {
      field: 'Mandatory',
      label: 'Mandatory (Always Granted)',
      Icon: Lock,
      description: 'When enabled, this cookie is always granted — the visitor cannot turn it off.',
      usage: 'Use only for cookies that are strictly necessary for the site to function: session tokens, CSRF protection, load balancing, UI preferences set by the user.',
      laws: [
        { name: 'ePrivacy Dir. Recital 25', text: '"Strictly necessary" cookies are exempt from consent. Only use this flag for cookies that genuinely meet this definition.' },
        { name: 'GDPR', text: 'Mandatory cookies still need to be disclosed in your privacy notice. Only the consent UI step is skipped.' },
        { name: 'ICO (UK)', text: 'Has challenged broad interpretation. Do not mark analytics or advertising cookies as mandatory.' },
      ],
      examples: 'session · csrf_token · load_balancer_id · cookie_consent_id · XSRF-TOKEN',
    },
    {
      field: 'Listen GPC',
...
export function CookieTemplateEditor({ id, current }: { id?: string; current: string }) {
  ...
  const [cookies, setCookies] = useState<TemplateCookie[]>(defaultCookies)
  ...
  useEffect(() => {
    if (!isNew && id) {
      cookieTemplatesApi.get(id)
        .then(t => { setName(t.name); setCookies(t.cookies as TemplateCookie[]) })
  ...
  const handleSave = async () => {
    ...
    if (cookies.some(c => !c.id.trim())) { setError('All cookies must have an ID.'); ok = false }
    if (duplicateIds.size > 0) { setError('Cookie IDs must be unique.'); ok = false }
    if (cookies.some(c => c.expiry < 1)) { setError('Expiry must be at least 1 day.'); ok = false }
    if (!ok) return
    ...
    if (!isNew && id) {
      ...
      const affected = profiles.filter(p => {
        ...
        return pj['cookieTemplateId'] === id || (pj['_meta'] as Record<string, unknown> | undefined)?.['cookieTemplateId'] === id
      })
      if (affected.length > 0) {
        const names = affected.map(p => `"${p.name}"`).join(', ')
        const confirmed = window.confirm(`This template is used by ${affected.length} profile${affected.length !== 1 ? 's' : ''}: ${names}.\n\nSaving will apply cookie changes to all of them. Continue?`)
        if (!confirmed) return
      }
    }
  ...
  return (
    <Layout title={isNew ? 'New Cookie Template' : 'Edit Cookie Template'} current={current}>
      {loading && <p class="text-sm text-gray-400">Loading template…</p>}
      {!loading && <div class="space-y-5">
        <div class="grid grid-cols-1 xl:grid-cols-[2.25fr_1.75fr] gap-5 items-start">
          <div class="bg-white border border-gray-200 rounded-lg p-5">
            <div class="py-5">
              ...
              <input
                ...
                placeholder="e.g. Google Analytics + Ads"
              />
            ...
            <div class="flex items-center justify-between mb-3">
              <div>
                <h3 class="text-sm font-semibold text-gray-700">Cookie Definitions</h3>
              ...
              <table class="w-full text-xs">
                <thead>
                  <tr class="text-left text-gray-500 border-b border-gray-200">
                    <th class="pb-2 pr-3 pl-1 font-medium">
                    ...
                    <th class="pb-2 pr-3 font-medium text-center">
                    ...
                    {tcfEnabled && <th class="pb-2 pr-3 font-medium text-blue-700">TCF Vendor</th>}
                    {showCpra && <th class="pb-2 pr-3 font-medium text-purple-700">CPRA Category</th>}
            ...
            {cookies.length === 0 && (
              <p class="text-sm text-gray-400 text-center py-4">
                No cookies defined. Click "+ Add Cookie" to start.
              </p>
            )}
          ...
          <div class="bg-white border border-gray-200 rounded-lg p-5 xl:sticky xl:top-4">
            <p class="text-xs text-gray-400 mb-3 uppercase tracking-wide font-semibold">Click a column header to learn more</p>
            ...
            {info && (
              <div class="space-y-3">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded p-3">
                  <p class="text-xs font-semibold text-blue-700 mb-1">Usage</p>
                  <p class="text-xs text-blue-800 leading-relaxed whitespace-pre-line">{info.usage}</p>
                </div>
                <div>
                  <p class="text-xs font-semibold text-gray-600 mb-2">Regulatory guidance</p>
                  <div class="space-y-2">
                    {info.laws.map((law, i) => (
                ...
                <div class="bg-gray-50 border border-gray-200 rounded p-2.5">
                  <p class="text-xs font-semibold text-gray-600 mb-1">Examples</p>
