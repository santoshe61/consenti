...
import type { DashboardProfile, ProfileSummary, ProfileVersionEntry, ComplianceValidationResult } from '@consenti/types'
...
export const profilesApi = {
  ...
  validateCompliance: (data: { complianceGroup: string; cookies: unknown[] }) =>
  ...
  getVersion: (id: string, version: number, locale = 'default') =>
    apiFetch<unknown>(`/profiles/${id}/versions/${version}?locale=${encodeURIComponent(locale)}`),
