...
export class NodeSqliteBuiltinAdapter implements StorageAdapter {
  ...
  async getConsents(filters: ConsentFilters): Promise<PagedResult<ConsentSummary>> {
    ...
    const rows = this.stmt(
      `SELECT ${NodeSqliteBuiltinAdapter.CONSENT_SUMMARY_COLS} FROM consent_records WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
    ).all(...vals, limit, offset) as unknown as RawConsentSummary[]
    const countRow = this.stmt(`SELECT COUNT(*) AS total FROM consent_records WHERE ${where}`).get(...vals) as unknown as RawCount
  ...
  async getVisitors(filters: VisitorFilters): Promise<PagedResult<Visitor>> {
    ...
    const countRow = this.stmt(`SELECT COUNT(*) AS total FROM visitors WHERE ${where}`).get(...vals) as unknown as RawCount
  ...
  async getLogs(filters: AuditFilters): Promise<PagedResult<AuditLogSummary>> {
    ...
    const offset = (page - 1) * limit
    const rows = this.stmt(
      `SELECT ${NodeSqliteBuiltinAdapter.AUDIT_SUMMARY_COLS} FROM audit_logs WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
    ).all(...vals, limit, offset) as unknown as RawAuditLogSummary[]
    const countRow = this.stmt(`SELECT COUNT(*) AS total FROM audit_logs WHERE ${where}`).get(...vals) as unknown as RawCount
