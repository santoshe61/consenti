...
interface RawProfileSummary {
  ...
  created_at: string; updated_at: string
  consent_template_id: string | null; ui_template_id: string | null
}
interface RawRole {
...
export class BetterSqlite3Adapter implements StorageAdapter {
  ...
  async connect(): Promise<void> {
    // Lazy: only loaded when this driver is selected — keeps better-sqlite3
    // as an optional peer dep without crashing startup for other drivers.
    const mod = await import('better-sqlite3').catch(() => {
      throw new Error("driver 'better-sqlite3' requires: npm install better-sqlite3")
    }) as { default: new(path: string) => DB }
    const { dbPath } = resolveStoragePaths(this.config.path ?? './consenti-data', 'better-sqlite3')
  ...
  async migrate(): Promise<void> {
    ...
    // kept as the hook for whenever a *real* future migration is needed: add `if (version < N)`
  ...
  async findActiveProfileByComplianceGroup(tenantId: string, complianceGroup: string): Promise<Profile | null> {
    const row = this.stmt(
      `SELECT * FROM profiles WHERE tenant_id = ?
       AND (json_extract(profile_json, '$.complianceGroup') = ? OR json_extract(profile_json, '$.customComplianceGroup') = ?)
       AND json_extract(profile_json, '$.isActive') = 1 LIMIT 1`
    ).get(tenantId, complianceGroup, complianceGroup) as RawProfile | undefined
    return row ? this.mapProfile(row) : null
  }
  ...
  async getConsents(filters: ConsentFilters): Promise<PagedResult<ConsentSummary>> {
    ...
    const rows = this.stmt(
      `SELECT ${BetterSqlite3Adapter.CONSENT_SUMMARY_COLS} FROM consent_records WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
    ).all(...vals, limit, offset) as unknown as RawConsentSummary[]
    const countRow = this.stmt(`SELECT COUNT(*) AS total FROM consent_records WHERE ${where}`).get(...vals) as RawCount
    return { items: rows.map(r => this.mapConsentSummary(r)), total: countRow.total, page, limit }
  }
  ...
  async getVisitors(filters: VisitorFilters): Promise<PagedResult<Visitor>> {
    ...
    const countRow = this.stmt(`SELECT COUNT(*) AS total FROM visitors WHERE ${where}`).get(...vals) as RawCount
    return { items: rows.map(r => this.mapVisitor(r)), total: countRow.total, page, limit }
  }
  ...
  async getLogs(filters: AuditFilters): Promise<PagedResult<AuditLogSummary>> {
    ...
    const offset = (page - 1) * limit
    const rows = this.stmt(
      `SELECT ${BetterSqlite3Adapter.AUDIT_SUMMARY_COLS} FROM audit_logs WHERE ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
    ).all(...vals, limit, offset) as unknown as RawAuditLogSummary[]
    const countRow = this.stmt(`SELECT COUNT(*) AS total FROM audit_logs WHERE ${where}`).get(...vals) as RawCount
    return { items: rows.map(r => this.mapAuditSummary(r)), total: countRow.total, page, limit }
  }

  async getAuditLogById(id: string): Promise<AuditLog | null> {
    const row = this.stmt('SELECT * FROM audit_logs WHERE id = ?').get(id) as RawAuditLog | undefined
  ...
  async updateConsentTemplate(id: string, data: UpdateConsentTemplateInput): Promise<ServerConsentTemplate> {
    ...
    const row = this.stmt('SELECT * FROM consent_templates WHERE id = ?').get(id) as RawConsentTemplate | undefined
