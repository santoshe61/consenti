    ...
    return (
      <div className="space-y-0">
        <CtrlGroup title="API Config">
          <CtrlRow>
            <Checkbox checked={api.enabled} onChange={(v) => setApi((p) => ({ ...p, enabled: v }))} label="Enable API mode" />
          </CtrlRow>
          <div className={api.enabled ? '' : 'opacity-40 pointer-events-none'}>
            <CtrlRow label="Base URL" hint="Backend API base URL. Defaults to /consenti/api.">
              <TextInput value={api.baseUrl} onChange={(v) => setApi((p) => ({ ...p, baseUrl: v }))} placeholder="https://example.com/consenti/api" />
            </CtrlRow>
  ...
  function renderProfileGpcBanner() {
    return (
      <CtrlGroup title="GPC Banner">
        <p className="ctrl-hint mb-3">
          Content for the GPC notification banner. Shown when <code className="bg-slate-100 px-0.5 rounded">autoHonorGPC: true</code> and a GPC signal is detected.
          GPC is detected via <em>navigator.globalPrivacyControl</em> — enable in Brave or Firefox+extension to test.
        </p>
        <CtrlRow label="Position">
          <Select
  ...
  return (
    <div className="max-w-7xl mx-auto bg-white border-t-4 border-brand-500 py-8 px-4 pb-48">
      ...
      <p className="text-xs text-slate-400 mb-5">
        Note: the playground passes settings via <code className="bg-slate-100 px-0.5 rounded">profileOverride</code> directly.{' '}
        <code className="bg-slate-100 px-0.5 rounded">ConsentiProfile</code> locale fallback (<code className="bg-slate-100 px-0.5 rounded">defaultLocale</code>) is not exercised here — locale switching always resolves to the configured locale.
      </p>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50">
          {(['setup', 'profile'] as MainTab[]).map((tab) => (
            <button
              ...
              className={`px-6 py-3 text-sm font-semibold border-b-2 transition-colors ${mainTab === tab
        ...
        <div className="flex min-h-[520px]">
          {mainTab === 'setup' ? (
            ...
            <>
              ...
              <div className={`flex-1 p-5 overflow-y-auto max-h-[700px] ${isDefault ? 'opacity-50 pointer-events-none' : ''}`}>
                {isDefault && (
                  <div className="mb-4 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                    Profile settings are ignored — "Use default banner profile" is enabled in Setup › Actions.
                  </div>
                )}
