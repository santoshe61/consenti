  ...
  return (
    <div className="prose max-w-none">
      ...
      <table>
        ...
        <tbody>
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Consent state</td></tr>
          ...
          <tr><td><code>isCookieGranted(cookieId, requestValue?)</code></td><td><code>boolean | ConsentStatus</code></td><td>Check if a single cookie is granted. Pass <code>true</code> as second arg to get the raw status string instead of a boolean</td></tr>
          <tr><td><code>isCategoryGranted(categoryId, requestValue?)</code></td><td><code>boolean | {'{'}[id: string]: ConsentStatus{'}'}[]</code></td><td>True when every cookie in the category is granted. Pass <code>true</code> to get an array of per-cookie status records</td></tr>
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Bulk actions</td></tr>
          <tr><td><code>grantAll(onlyMandatory?)</code></td><td><code>Promise&lt;void&gt;</code></td><td>Accept all cookies. Pass <code>true</code> to grant only mandatory and deny the rest</td></tr>
          <tr><td><code>denyAll(includingMandatory?)</code></td><td><code>Promise&lt;void&gt;</code></td><td>Deny all non-mandatory cookies. Pass <code>true</code> to deny mandatory too (logs a warning)</td></tr>
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Visibility</td></tr>
          ...
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Events</td></tr>
          <tr><td><code>on(event, handler)</code></td><td><code>void</code></td><td>Subscribe to a typed widget event. The <code>consenti:</code> prefix is optional</td></tr>
          <tr><td><code>off(event, handler)</code></td><td><code>void</code></td><td>Unsubscribe a previously registered handler (pass the same function reference)</td></tr>
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Lifecycle</td></tr>
          ...
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Runtime configuration</td></tr>
          <tr><td><code>setDarkMode(enable?)</code></td><td><code>void</code></td><td>Toggle or set dark mode without re-initialising. Omit <code>enable</code> to toggle</td></tr>
          <tr><td><code>setTheme(theme)</code></td><td><code>void</code></td><td>Merge CSS token overrides into the current theme at runtime</td></tr>
          <tr><td><code>setConfig(config)</code></td><td><code>void</code></td><td>Deep-merge a partial config. Re-applies theme/dark mode side-effects; does not re-init</td></tr>
          <tr><td><code>setProfile(override)</code></td><td><code>void</code></td><td>Merge a partial profile override and re-render visible UI without a network call</td></tr>
          <tr><td colSpan={3} className="bg-gray-50 font-semibold text-xs uppercase tracking-wide text-gray-500 px-2 py-1">Diagnostics</td></tr>
          <tr><td><code>version()</code></td><td><code>{'{'} package, profileVersion, consentVersion {'}'}</code></td><td>Package version and active profile/consent version numbers</td></tr>
        </tbody>
      </table>
...
// }`} />
  ...
  widget.reConsent()
      ...
      <CodeBlock lang="ts" code={`// Boolean mode (default) — true if granted
...
// 'granted' | 'denied' | 'objected' | false`} />
      <p>During SSR or before <code>init()</code> completes, always returns <code>false</code>.</p>

      <h3>isCategoryGranted(categoryId, requestValue?)</h3>
      <p>
        Category-level consent check. The <code>categoryId</code> must match a <code>Category.id</code>
        in the profile's preference modal.
      </p>
      <CodeBlock lang="ts" code={`// Boolean mode — true only when ALL cookies in the category are 'granted'
...
// [{ ad_storage: 'granted' }, { ad_personalization: 'denied' }]`} />
      <p>
        Returns <code>false</code> / <code>[]</code> if the category ID is not found or the widget is not yet initialised.
      </p>

      <h3>grantAll(onlyMandatory?)</h3>
      <p>Programmatically accept cookies without the user interacting with the banner. Dismisses the banner after submitting.</p>
      <CodeBlock lang="ts" code={`// Accept everything
...
await widget.grantAll(true)`} />
      ...
      <CodeBlock lang="ts" code={`// Deny non-mandatory; mandatory cookies stay 'granted'
await widget.denyAll()

// Deny everything including mandatory — use with caution
await widget.denyAll(true)`} />
      <Callout type="warning">
        Passing <code>true</code> to <code>denyAll</code> denies mandatory cookies. This is intentionally allowed
        for testing and special integrations, but will log a <code>console.warn</code> as a reminder.
      </Callout>
...
// 'consentSubmitted'        — consent saved (cookie written + API call if configured)`} />
      <Callout type="info">
        <code>on()</code> / <code>off()</code> use the same underlying DOM events as
        raw <code>window.addEventListener</code> calls. Both can coexist in the same page.
        Registered handlers are automatically cleaned up when <code>destroy()</code> is called.
      </Callout>

      <h3>setDarkMode(enable?)</h3>
      <p>Toggle or set dark mode at runtime without re-initialising the widget. The dark class is applied to both the banner root and the preference modal.</p>
      <CodeBlock lang="ts" code={`widget.setDarkMode()       // toggle current state
...
mq.addEventListener('change', (e) => widget.setDarkMode(e.matches))`} />
      ...
      <p>Merge CSS token overrides into the current theme at runtime. CSS custom properties on the root element update immediately — no page reload required.</p>
      <CodeBlock lang="ts" code={`// Switch primary colour on the fly (e.g. white-label tenant switch)
...
widget.setTheme({ borderRadius: '0px' })`} />
      ...
      <p>
        Deep-merges a partial <code>ConsentiConfig</code> into the current config without re-initialising.
        Theme and dark mode side-effects are re-applied immediately. For locale or profile changes,
        follow up with <code>switchLocale()</code> or <code>init()</code>.
      </p>
      <CodeBlock lang="ts" code={`// Update theme and dark mode together
widget.setConfig({
  darkMode: true,
...
widget.setConfig({ hidePoweredBy: true })`} />
      ...
      <CodeBlock lang="ts" code={`// Change the banner heading live
widget.setProfile({
...
widget.setProfile({
  preferenceModal: { position: 'left' },
      ...
      <h3>version()</h3>
      <p>Returns the package version and the currently active profile and consent version numbers. Useful for support diagnostics and feature flags.</p>
      <CodeBlock lang="ts" code={`const info = widget.version()
// {
...
// }
console.log(\`Consenti \${info.package} | profile v\${info.profileVersion}\`)`} />

      <h3>destroy()</h3>
      <p>Removes the banner DOM, modal DOM, BroadcastChannel listener, and all event listeners (including those registered via <code>on()</code>). Call when unmounting in SPA route changes if you manage the lifecycle manually.</p>
