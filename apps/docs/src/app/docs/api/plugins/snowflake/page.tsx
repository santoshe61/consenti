...
export const metadata: Metadata = { title: 'Snowflake Plugin' }
...
export default function SnowflakePluginPage() {
  return (
    <div className="prose max-w-none">
      ...
      <p>
        Load consent records into Snowflake for enterprise data warehousing,
        compliance reporting, and cross-system joins.
      </p>
      ...
      <h2>Configuration</h2>
      <CodeBlock lang="ts" code={`import { SnowflakePlugin } from '@consenti/api/plugins'
...
createConsenti({
  plugins: [
    new SnowflakePlugin({
      ...
      <CodeBlock lang="sql" code={`CREATE TABLE CONSENTI.PUBLIC.CONSENT_RECORDS (
...
export class SnowflakePlugin extends ConsentiPlugin {
  ...
  override async afterConsentSave(record: ConsentDbRecord): Promise<void> {
    const sql = \`
      ...
      VALUES (:1, :2, :3, :4, PARSE_JSON(:5), :6, :7, :8::TIMESTAMP_TZ)
    \`
    await new Promise<void>((resolve, reject) => {
  ...
  override async destroy(): Promise<void> {
    await new Promise<void>(resolve => {
      ...
      <ol>
        ...
        <li>Flush every 60 seconds via <code>setInterval</code> in <code>initialize()</code></li>
        <li>Use Snowflake's <code>COPY INTO</code> from an S3/GCS stage for maximum throughput</li>
      </ol>

      <Callout type="info">
        Snowflake's Time Travel (default: 1 day, max 90 days on Enterprise) provides point-in-time rollback
        for regulatory investigations. Set <code>DATA_RETENTION_TIME_IN_DAYS = 90</code> on the consent table
        to maximise your audit window.
      </Callout>
    </div>
  )
}
